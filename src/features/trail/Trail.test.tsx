...
describe("Trail", () => {
  ...
  it("oferece um único botão flutuante e abre sem seleção", () => { setup(); const button = screen.getByRole("button", { name: /Começar treino/ }); expect(button).toBeVisible(); fireEvent.click(button); expect(screen.getByText("Escolha as trilhas")).toBeVisible(); expect(screen.getAllByRole("button", { pressed: false })).toHaveLength(4); expect(screen.queryByRole("button", { name: /Começar treino/ })).not.toBeInTheDocument(); });
