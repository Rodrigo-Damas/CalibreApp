...
describe("WorkoutFlow", () => {
  it("começa pelas trilhas e não pergunta por energia ou disposição", () => { setup(); expect(screen.getByText("Escolha as trilhas")).toBeVisible(); expect(screen.getAllByRole("button", { pressed: false })).toHaveLength(4); expect(screen.queryByText(/tanque|energia|disposição/i)).not.toBeInTheDocument(); });
  it("avança diretamente para uma ficha completa", () => { setup(); choose("Empurrar"); expect(screen.getByRole("heading", { name: "Empurrar", level: 2 })).toBeVisible(); expect(screen.getByText("Flexão no solo")).toBeVisible(); expect(screen.getByText("5 séries × 14 repetições")).toBeVisible(); expect(screen.getByText("70 repetições estimadas")).toBeVisible(); });
