...
export function WorkoutFlow({ onClose, initialTrack, initialTracks }: { onClose: () => void; initialTrack?: TrackId; initialTracks?: TrackId[] }) {
  ...
  return <div className={`workout-builder${step === "running" ? " timer-active" : ""}`} role="dialog" aria-modal="true" aria-label={step === "running" ? "Cronômetro" : "Montar treino"} data-reduced-motion={store.preferences.reducedMotion || undefined}>
    {step !== "running" && <header className="builder-header">
      ...
      <div className="builder-header__title"><small>NOVO TREINO</small><h1>Montar treino</h1><div className="builder-progress" aria-label={`Etapa ${stepNumber} de 2`}>{[1, 2].map((item) => <i key={item} className={item <= stepNumber ? "is-filled" : ""} />)}</div></div>
      <button onClick={onClose} aria-label="Fechar"><X aria-hidden="true" /></button>
    </header>}
    <main>
      {step === "tracks" && <section className="builder-step"><p className="step-label">1 · TRILHAS</p><h2>O que entra neste treino?</h2><TrackSelector selected={selected} onToggle={toggle} repeated={repeated} />{selected.some((item) => repeated.includes(item)) && <aside className="recovery-note">Você já treinou esta trilha hoje. Você ainda pode montar outra sessão.</aside>}<button className="primary-button" disabled={!selected.length} onClick={() => setStep("build")}>Montar ficha</button></section>}
      {step === "build" && <section className="builder-step build-sheet"><header><p className="step-label">2 · FICHA</p><h2>Sua sessão</h2><p>{selected.map((id) => tracks.find((track) => track.id === id)!.name).join(" · ")}</p></header>
        <section className="build-section" aria-labelledby="duration-title"><h3 id="duration-title">Duração</h3><div className="duration-segmented" role="group" aria-label="Duração do treino">{(["short", "long"] as DurationChoice[]).map((choice) => <button key={choice} aria-pressed={duration === choice} onClick={() => setDuration(choice)}>{choice === "short" ? "Curto" : "Longo"}<small>{SETS[choice]} séries por trilha</small></button>)}</div></section>
        {selected.length > 1 && <section className="build-section"><h3>Formato</h3><div className="format-segmented" role="group" aria-label="Formato do treino"><button aria-pressed={format === "blocks"} onClick={() => setFormat("blocks")}><strong>Blocos</strong><span>Uma trilha por vez</span></button><button aria-pressed={format === "circuit"} onClick={() => setFormat("circuit")}><strong>Circuito</strong><span>Alternar a cada minuto</span></button></div></section>}
        <section className="build-section"><h3>Prescrições</h3><div className="prescription-list">{prescriptions.map((prescription) => { const track = tracks.find((item) => item.id === prescription.track)!; const known = store.sessions.flatMap((session) => session.prescriptions.filter((item) => item.track === prescription.track).map((item) => item.chosenShortReps)); return <article key={prescription.track} style={{ "--track": track.color } as CSSProperties}><div className="prescription-heading"><small>{track.name}</small><h4>{track.exercise}</h4><p><strong>{prescription.sets} séries × {prescription.displayedReps} repetições</strong><br />{prescription.estimatedVolume} repetições estimadas</p></div><div className="dose-control"><button aria-label={`Diminuir repetições de ${track.name}`} onClick={() => changeReps(track.id, -1)}>−</button><strong aria-label={`${prescription.displayedReps} repetições por série`}>{prescription.displayedReps}</strong><button aria-label={`Aumentar repetições de ${track.name}`} onClick={() => changeReps(track.id, 1)}>+</button></div><p className="context-message">{contextualMessage(prescription.suggestedShortReps, prescription.chosenShortReps, known)}</p>{isLargeIncrease(prescription.suggestedShortReps, prescription.chosenShortReps) && <button className="reset-suggestion" onClick={() => setChosen((value) => ({ ...value, [track.id]: prescription.suggestedShortReps }))}>Voltar à sugestão</button>}</article>; })}</div></section>
        <section className="build-section build-summary" aria-live="polite"><h3>Resumo</h3><strong>{selected.length * SETS[duration]} minutos estimados</strong><span>{selected.length > 1 ? (format === "blocks" ? "Em blocos" : "Em circuito") : "Trilha individual"} · {prescriptions.reduce((sum, item) => sum + (item.estimatedVolume ?? 0), 0)} repetições</span><WorkoutSequence prescriptions={prescriptions} mode={format} /></section>
