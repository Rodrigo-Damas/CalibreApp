...
export function MockStoreProvider({children}:{children:React.ReactNode}){
  ...
  const value=useMemo<Store>(()=>({xp:2840,streak:7,completed,minutes:0,repetitions,session,lastSession,journeys,
    ...
    finishSession(results){setLastSession(results);setCompleted(v=>v+results.length);setRepetitions(v=>v+results.reduce((sum,r)=>sum+r.completedVolume,0));setJourneys(current=>current.map(track=>{const related=results.filter(r=>r.practice.track===track.id);if(!related.length)return track;const values=related.map(r=>r.completedVolume);const added=values.reduce((a,b)=>a+b,0);return {...track,history:[...track.history,...values],personalBest:Math.max(track.personalBest,...values),accumulated:track.accumulated+added,weekly:track.weekly+added}}));setSession([])}
